...
<a href="http://www.youtube.com/watch?feature=player_embedded&v=PLlmKjzN-qBWnpZJk38LIyBAQRyYTl1iee" target="_blank"><img src="http://img.youtube.com/vi/Y0-Es4A7FK8/0.jpg" alt="Youtube video series about SDF" width="240" height="180" border="10" /></a>
...
> 
> 💡 _Naked type variables (in return type or in argument) is not allowed. TypeVariables must only be used either with `@Bind` or with `@Realtime`._
...
SDF supports resolving dependencies with parameters provided at the runtime. This helps initializing a class instance with different runtime values. The parameter which will be supplied at the runtime needs to be annotated with `@Realtime` with an id. At the time of resolving the class, `dynamicResolve()` needs to be used with a map having the parameter value against the id as key.
```java
@Dependency
public class UserAnalysisService {
  ...
  public UserService(IAdapter adapter, @Realtime(id = "user") User user) {
    this.adapter = adapter;
    this.user = user;
  }
...
```
> 💡 _Class with `@Realtime` parameters cannot be used as a dependency for another class_
...
Profile can also be passed in JVM argument like `sdf.<context>.profile=<profile>` where `<context>` is the context name for which the `<profile>` will be set. For example, to pass a profile name "dev" to default context, `sdf.default.profile=dev` needs to be passed in JVM argument.
> 💡 _When profile is passed in both, JVM argument and DependencyResolverConfig, JVM argument gets higher precedence_
...
## Constraints/Restrictions
  ...
  * Naked type variables (in return type or in argument) is not allowed. TypeVariables must only be used either with `@Bind` or with `@Realtime`..
  * Only `List` and `Map` types are supported for Subtypes.
  * Class with `@Realtime` parameters cannot be used as a dependency for another class.
